...
begin
	...
	s_D(180 downto 0) <= (others => '0') when i_Flush = '1' else i_D(180 downto 0);
	s_D(219 downto 188) <= (others => '0') when i_Flush = '1' else i_D(219 downto 188);
	s_D(187 downto 181) <= "0010011" when i_Flush = '1' else i_D(187 downto 181); --sets opcode to addi to complete NOP instruction
    ...
    ALUA_dffgs : for i in 15 to 46 generate
	A_dffg : dffg
	  port map(i_CLK => i_CLK,     -- Clock input
     		   i_RST => i_RST,     -- Reset input
  	           i_WE => i_WE,     -- Write enable input
    ...
    ALUB_dffgs : for i in 47 to 78 generate
	B_dffg : dffg
	  port map(i_CLK => i_CLK,     -- Clock input
     		   i_RST => i_RST,     -- Reset input
  	           i_WE => i_WE,     -- Write enable input
    ...
    ALUimm_dffgs : for i in 79 to 110 generate
	imm_dffg : dffg
	  port map(i_CLK => i_CLK,     -- Clock input
     		   i_RST => i_RST,     -- Reset input
  	           i_WE => i_WE,     -- Write enable input
	...
	Inst_dffgs : for i in 156 to 187 generate
	i_dffg : dffg
	  port map(i_CLK => i_CLK,     -- Clock input
     		   i_RST => i_RST,     -- Reset input
  	           i_WE => i_WE,     -- Write enable input
	...
	fetchInstAdd_dffgs : for i in 188 to 219 generate
	i_dffg : dffg
	  port map(i_CLK => i_CLK,     -- Clock input
     		   i_RST => i_RST,     -- Reset input
  	           i_WE => i_WE,     -- Write enable input
